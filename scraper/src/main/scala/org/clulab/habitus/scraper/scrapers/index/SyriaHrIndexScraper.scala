...
import org.clulab.habitus.scraper.domains.SyriaHrDomain
import org.clulab.habitus.scraper.scrapes.IndexScrape

class SyriaHrIndexScraper extends PageIndexScraper(SyriaHrDomain) {

  def scrape(browser: Browser, page: Page, html: String): IndexScrape = {
    val doc = browser.parseString(html)

    // Extract article links based on SyriaHR's HTML structure
