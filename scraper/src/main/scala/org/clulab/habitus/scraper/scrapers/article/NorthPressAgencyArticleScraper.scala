...
class NorthPressAgencyArticleScraper extends PageArticleScraper(NorthPressAgencyDomain) {
  ...
  def scrape(browser: Browser, page: Page, html: String): ArticleScrape = {
    ...
    // Extracting Author
    val author = (doc >?> element("meta[name='author']")).map(_.attr("content")).orElse(Some("North Press Agency"))
